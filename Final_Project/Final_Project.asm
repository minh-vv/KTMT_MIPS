...
check_length:
        ...
        beqz $s4, exit		# Neu string rong thi exit
# In dong 1
        li $v0, 4
        la $a0, in1
        syscall
...
error_length:           	# thong bao loi khong chia het cho 8
        li $v0, 4
        la $a0, error_msg
        syscall
        j input
...
block11: # luu ki tu vao disk 1
        lb $t1, ($s0)
        sb $t1, ($s1)
block21: # Luu ki tu vao disk 2 
        lb $t2, 4($s0)
        sb $t2, ($s2)
parity1: # Luu xor vao parity (disk 3)
        xor $t3, $t1, $t2
        sb  $t3, 0($s3)
...
print31:
        ...
        addi $t0, $0, 0
prbk31: # In disk 3
        lb $t8, ($s3)		# Truyen lan luot tung xor dang nhi phan vao ham Hex => dang hexa
        jal HEX
        li $v0, 4
        la $a0, inphay 		# ,
        syscall
...
end1:
        ...
        addi $s0, $s0, 4		# Tang dia chi input len 4 byte
...
split2:    
        ...
        addi $t0, $0, 0        	# so ki tu doc moi block
block12: # luu ki tu vao disk 1
        lb $t1, ($s0)
        sb $t1, ($s1)
block22: # Luu ki tu vao disk 3 
        lb $t2, 4($s0)
        sb $t2, ($s3)
parity2: # Luu xor vao parity (disk 2)
        xor $t3, $t1, $t2 
        sb  $t3, 0($s2)
...
block23: # Luu ki tu vao disk3 
        lb $t2, 4($s0)
        sb $t2, ($s3)
parity3: # Luu xor vao parity (disk 1)
        xor $t3, $t1, $t2 
        sb  $t3, 0($s1)
...
#-----HEX-----
HEX:    li $t4, 1			
loop:   blt $t4, $0, endloop
        sll $s6, $t4, 2        	# s6 = t4*4
        srlv $a0, $t8, $s6      # a0 = t8>>s6
        andi $a0, $a0, 0xf     	# a0 = a0 & 0000..1111 => lay 4 bit cuoi cung cua a0
        
        la $t7, hex 
        add $t7, $t7, $a0 
        lb $a0, 0($t7)          # print hex
        li $v0, 11
        syscall
